...
from app.domain.environment.models import EnvironmentEvent
from app.domain.platform import service as platform
from app.domain.tasks.models import Assignment, MachineConfirmation, Task, TaskSession
from app.domain.tasks.rules import CONFIRMATION_VALID_FOR
from app.domain.telemetry.models import MachineTelemetry
from app.domain.twin import fusion
from app.domain.twin.models import TwinIntelligence, TwinSnapshot
...
# ---------------- read views (used by the API and Copilot tools) ----------------


async def machine_view(machine_id: str):
    """Machine master data + live state as a TwinMachineState."""
    from app.core.exceptions import NotFound
    from app.domain.twin.schemas import TwinMachineState
...
async def site_twin(site_id: str):
    """All active operator twins in a site + every machine that has reported telemetry there."""
    from app.domain.twin.schemas import SiteTwin
    ...
    async with rt.db.sessionmaker() as s:
        ...
        ops |= set((await s.scalars(select(TaskSession.operator_id).where(
            TaskSession.site_id == site_id, TaskSession.status.in_(("IN_PROGRESS", "PAUSED"))))).all())
        machine_ids = set((await s.scalars(select(MachineTelemetry.machine_id).where(MachineTelemetry.site_id == site_id)
                                           .distinct())).all())
    ops |= set(await svc.operators_in_site(site_id))
    twins = [t for op in sorted(ops) if (t := await svc.get_twin(op))]
