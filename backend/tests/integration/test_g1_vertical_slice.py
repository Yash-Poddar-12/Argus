...
def test_g1_vertical_slice(client, runtime, monkeypatch):
    ...
    from app.domain.tasks import lifecycle as task_lifecycle

    last_ts = datetime.fromisoformat(points[-1]["ts"].replace("Z", "+00:00"))
    monkeypatch.setattr(task_lifecycle, "utcnow", lambda: last_ts + timedelta(seconds=5))  # complete after last reading
