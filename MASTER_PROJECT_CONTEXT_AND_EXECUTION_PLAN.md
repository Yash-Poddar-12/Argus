...
> | **Modular parallel workflow** (phases, gates, conflict-free rules) | `docs/01-architecture/MODULAR_WORKFLOW.md` |
> | Who owns which folder | `docs/01-architecture/REPO_STRUCTURE.md` |
> | Module scope + work packages | `docs/03-modules/MXX-*/SPEC.md` |
> | Live module status (updated on every push) | `docs/03-modules/MXX-*/STATUS.md` + `python scripts/status.py board` |
> | Push/pull status marking | `docs/04-workflow/SYNC_PROTOCOL.md` |
> | Exact interfaces | `contracts/**` + `docs/02-contracts/*.md` |
...
(Path namespaced under `/sessions/` so it can't collide with `/training/{training_id}/…`. Full ownership-annotated list: `docs/02-contracts/API_CATALOG.md`.)
...
Setup per tool and ready-made starter prompts: `docs/04-workflow/AGENT_PLAYBOOK.md`.
...
5. docs/01-architecture/REPO_STRUCTURE.md          (which paths the module may edit)
6. docs/01-architecture/SYSTEM_ARCHITECTURE.md     (as needed)
7. docs/03-modules/MXX-*/SPEC.md and STATUS.md     (relevant module)
8. relevant contracts: contracts/** + docs/02-contracts/*.md
```
...
- Every module's first work package is **WP0: contract + `public.py` facade + mocks**, so consumers are unblocked within about a day of a module starting.
- M00 and M01 are themselves split into work packages that live in **different folders**, so 3–5 people can build the base together without conflicts (`docs/01-architecture/MODULAR_WORKFLOW.md` §3).
- Parallel safety comes from **one owner per path** (`docs/01-architecture/REPO_STRUCTURE.md`), **auto-discovery** of backend modules, **pre-registered frontend slots**, per-module migrations/compose/env files, and **per-module status files** (MODULAR_WORKFLOW §5).
...
- backend module registry (auto-discovery of `backend/modules/*/module.py`),
- WebSocket gateway,
...
Spec and work packages: `docs/03-modules/M00-foundation/SPEC.md`.
...
- telemetry ingestion,
- simulator core (other modules add scenarios under `simulator/scenarios/<module>/`),
...
Spec and work packages: `docs/03-modules/M01-operational-twin/SPEC.md`.
...
Every module specification must use this structure. A ready-to-copy version is in `docs/03-modules/_TEMPLATE/` (SPEC.md + STATUS.md), and all twelve module specs (M00–M11) exist under `docs/03-modules/`.
...
> The allocation actually in use (names, handles, slot holders, per-phase plan) lives in `docs/00-project/TEAM_AND_OWNERSHIP.md`, which also includes Phase 0/1 work-package splits so nobody is idle while the base is built. The tables below are the original starting point.
...
Exact implementation may evolve, but ownership must be explicit. The full table-by-table list is in `docs/02-contracts/DATA_OWNERSHIP.md`.
...
| Copilot tools | Owner of the underlying data | M07 owns the registry and orchestration (`docs/02-contracts/TOOL_AND_ML_CONTRACTS.md`) |
| Frontend feature slots (copilot, training, site-intel, scenarios, analytics) | App owner (M02/M03) pre-registers; slot is delegable to the feature module | `docs/01-architecture/REPO_STRUCTURE.md` §3 |
...
Versioning, the breaking-change process, mocks and contract tests: `docs/02-contracts/CONTRACTS_GUIDE.md`. Catalogs: `API_CATALOG.md`, `EVENT_CATALOG.md`, `DATA_OWNERSHIP.md`, `TOOL_AND_ML_CONTRACTS.md` in the same folder.
...
[ ] Demo path verified
[ ] Module STATUS.md updated (see §116)
```

Levels (work package, module, contract change, safety-relevant change): `docs/04-workflow/DEFINITION_OF_DONE.md`. Gate checklists: `docs/04-workflow/INTEGRATION_CHECKPOINTS.md`.
...
Decision:          Introduced the multi-IDE modular workflow documentation set: AGENTS.md (canonical) + CLAUDE.md/GEMINI.md
                   ...
                   Event contracts foldered by producing module (contracts/events/mXX/); one OpenAPI file per module.
                   Each module exposes a public.py facade; WP0 of every module = contract + facade + mocks.
...
The full method is in `docs/01-architecture/MODULAR_WORKFLOW.md`. The short version:
...
PHASE 2+ M02 M03 M04 M06 → M05 M07 M08 → M09 M10 M11                  → Gates G2–G4 = demo scenes 1–12
         all in parallel, talking only through contracts/ + public.py facades + mocks
```
...
Before push:  python scripts/status.py log --module MXX --action PUSH --msg "..." [--wp MXX-WPn=STATE]
              → updates docs/03-modules/MXX-*/STATUS.md + your sync log; commit it WITH the code
...
Each status file has exactly one writer (module owner or the person themselves), so status tracking never causes merge conflicts. Details: `docs/04-workflow/SYNC_PROTOCOL.md`.
