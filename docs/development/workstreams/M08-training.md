...
- Training catalogue (content in `backend/app/domain/training/content/`, metadata in DB): category, machine_type, skill_area, difficulty, duration, format, passing score
...
`backend/app/domain/training/` (static content under `training/content/`), `backend/app/api/v1/training.py`, `contracts/events/training/`, `frontend/src/features/training/`.
...
| WP | Title | Est. |
|----|-------|------|
| M08-WP0 | Contracts + schemas + service + mocks (recommendation with reason, impact example) | 1 d |
...
|----|-------|-------|-------------|-------|
| M08-WP0 | Contracts + schemas + service + mocks (recommendation with reason, impact example) | NOT_STARTED | — | |
...
|------------|-------------|---------------------|
| M06 anomaly/skill/deviation | `contracts/events/<area>/examples` | M06-WP4/WP6/WP7 LIVE |
