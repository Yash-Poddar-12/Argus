...
|------|-----------|-----------------|
| `docs/development/workstreams/MXX-*.md` | **Only that workstream's owner** (Status section; the plan above it changes by reviewed PR) | Workstream state, WP states, mocks in use, blockers, contract changes, **Update log** (one line per push) |
| `docs/development/sync-log/<handle>.md` | **Only that person** | Personal log of every PULL and PUSH, plus notes on what changed for you |
| `docs/development/workstreams/README.md` | Nobody (static index) | Links + instructions; live values come from `python scripts/status.py board` |
...
# 2. mark status (module workstream status + your sync log in one go)
...
- Inserts the same entry into `docs/development/sync-log/<you>.md`

**No Python / prefer manual?** Edit the same three spots by hand. The format is shown in each workstream status. Newest line goes directly under `<!-- log:insert -->`.
...
`changes` shows:
- changed files in `contracts/`, `docs/architecture/contracts/`, `docs/development/workstreams/`, the master plan, and `AGENTS.md`
...
| `MOCKED` | Contract + schemas + service + mocks shipped; consumers can build against it (WP0 done) |
...
- At the **start** of a session: run `python scripts/status.py changes` (or `git log` on the watched paths) and read your module's workstream status.
...
- Never edit another module's workstream status or another person's sync log.
...
#!/bin/sh
git diff --name-only @{u}.. 2>/dev/null | grep -q "docs/development/workstreams/ || {
  echo "Reminder: no workstream status change in this push. Run: python scripts/status.py log --module MXX --action PUSH --msg ..."
}
exit 0
```

CI can later add a soft check that warns when a PR touches a module's paths without touching its workstream status.
