...
- [ ] Every teammate: fresh clone → `python scripts/dev.py up` → healthy on their own machine/OS
...
- [ ] Both frontend shells render all role-guarded area pages
...
- [ ] Every module's workstream status has an owner
...
- [ ] Every Phase-2 module has shipped **WP0** (contract + schemas + service + mocks) or has a date for it
...
- [ ] Full demo rehearsal from a clean `python scripts/dev.py up` + seed + scripted simulator run
