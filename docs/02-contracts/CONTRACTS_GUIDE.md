...
```json
{
  ...
  "payload": { "...": "validated by contracts/events/<producer mXX>/<event_type>.v1.json" }
}
```
...
4. Record it in your `STATUS.md` → "Contract changes" and in the catalog.
...
- Each module's `public.py` accessor returns the mock when `MXX_USE_MOCK=true` or the real implementation isn't registered.
...
- Frontend mocks: `frontend/apps/<app>/src/features/<feature>/mocks/` import JSON from `contracts/**/examples/`.
...
| Client drift | M00 CI | `frontend/packages/api-client` matches what the OpenAPI files generate |
