...
> **One writer per table / key space / bucket.** Other modules read through the owner's API, events, `public.py` facade, or a read model they build from events. They never `INSERT`/`UPDATE`/`DELETE` another module's tables and never write SQL joins across modules. Source: master §27–§34, §57.
>
