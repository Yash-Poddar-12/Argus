...
Predictions and explanations are the "Understand → Predict" part of the product thesis. Putting all ML behind one module's `public.py` means frontend and Copilot never depend on model internals (master §26), and models can be swapped (P12).
...
- Model registry (versions, metrics, model cards), inference serving via `public.py` + REST, inference cache
...
`ml/` (except `ml/federated/`), `backend/modules/m06_ml/`, `backend/migrations/m06_ml/`, `backend/tests/m06_ml/`, `simulator/scenarios/behavior/`, `contracts/ml/`, `contracts/openapi/m06-predictions.yaml`, `contracts/events/m06/`, `contracts/tools/{predict_task_duration,get_operator_performance,explain_operational_deviation}.json`.
...
| WP | Title | Est. |
|----|-------|------|
| M06-WP0 | Contracts + `public.py` + mocks for all 5 interfaces (shipped first, unblocks M01/M02/M07/M09) | 1 d |
