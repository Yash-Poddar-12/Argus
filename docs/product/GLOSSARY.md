...
| **Auto-discovery** | Core loads every `backend/app/domain/*/wiring.py` automatically, so nobody edits a central router. |
...
| **workstream status** | A module's live status and append-only update log. Updated on every push. |
| **Sync log** | A person's own log of pulls and pushes (`docs/development/sync-log/<handle>.md`). |
| **ADR** | Architecture Decision Record (`docs/architecture/decisions/`). |
