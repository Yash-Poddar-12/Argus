...
Before coding:
1. Read AGENTS.md, then docs/01-architecture/MODULAR_WORKFLOW.md and REPO_STRUCTURE.md.
2. Read docs/03-modules/[MXX-folder]/SPEC.md and STATUS.md.
3. Read the contracts this WP touches (contracts/** and docs/02-contracts/*.md).
...
with at least one example each (normal + one edge case), backend/modules/[mxx]/public.py exposing DTOs, Protocols
...
[MYY]'s public.py / API. Keep the mock available for tests. Update STATUS.md "Using mocks for".
```
...
Copilot answers not grounded in tool results, missing tests, missing STATUS.md update.
```
...
- **Generated code:** never let an agent hand-edit `frontend/packages/api-client`; run `make contracts`.
- **Lockfiles:** agents should not hand-merge them. Regenerate instead.
- **Secrets:** agents must use placeholders in `infra/env/*.env.example`; real keys stay in your local `.env` (gitignored).
