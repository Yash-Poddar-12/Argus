...
| UI | `frontend/src/components/` (Tailwind + headless primitives), MapLibre GL for site map, Recharts for charts | design tokens |
...
| Local run | Docker Compose (`docker-compose.yml` + per-module files), `Makefile` wrappers | — |
...
- **UI styling:** plain CSS design tokens in `frontend/src/components/src/tokens.css` instead of Tailwind (fewer moving parts across teammates' toolchains). Revisit if M02/M03 prefer Tailwind.
