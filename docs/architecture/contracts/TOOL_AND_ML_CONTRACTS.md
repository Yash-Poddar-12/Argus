...
**Ownership split:**
- **Tool schema + implementation** belong to the module that owns the underlying data (`contracts/tools/<tool_name>.json`, implemented in that module's `schemas.py`/`service.py`).
...
Owners implement tools behind their `schemas.py`/`service.py` (M01: `call_tool(name, arguments)`), and M07 calls them there.
...
All ML sits behind these interfaces (master §26). Frontend, Copilot, and other modules never depend on model internals. Schemas: `contracts/ml/*.v1.json`. Access: `backend.app.intelligence (interfaces)`.
