#!/usr/bin/env python3
"""Workstream status helper for parallel, multi-IDE teamwork.

Stdlib only; works on Windows, macOS and Linux.

  python scripts/status.py board [--markdown]
      Live board computed from every docs/development/workstreams/M*.md (Status section).

  python scripts/status.py log --action PUSH --msg "..." [--module M02]
                               [--wp M02-WP1=DONE ...] [--state IN_PROGRESS] [--handle you]
      With --module: updates that workstream's Status section (Last updated, optional WP/workstream
      state, new line under "Update log"). Always appends to your own sync log
      docs/development/sync-log/<handle>.md.

  python scripts/status.py changes [--since REF]
      After a pull: which contracts, docs and workstream statuses changed since REF
      (default ORIG_HEAD, which git sets on pull/merge/rebase).

See docs/development/SYNC_PROTOCOL.md.
"""
...
MODULES_DIR = ROOT / "docs" / "development" / "workstreams"
SYNC_DIR = ROOT / "docs" / "development" / "sync-log"
...
WATCHED = ["contracts", "docs/architecture", "docs/development/workstreams", "docs/product", "AGENTS.md"]
...
def find_status(module: str) -> Path:
    ...
    hits = sorted(MODULES_DIR.glob(f"{mod}-*.md"))
    if not hits:
        sys.exit(f"No workstream file found for {mod} under {MODULES_DIR}")
    return hits[0]
...
def _split_status(text: str) -> tuple[str, str]:
    """Workstream files are '<plan>\\n# Status\\n<status>'; edits only ever touch the status part."""
    if STATUS_HEADING not in text:
        return "", text
    plan, status = text.split(STATUS_HEADING, 1)
    return plan + STATUS_HEADING, status


def set_table_value(text: str, key: str, value: str) -> str:
    plan, status = _split_status(text)
    pat = re.compile(rf"^(\|\s*{re.escape(key)}\s*\|)[^|\n]*(\|)", re.M)
    if not pat.search(status):
        sys.exit(f"Could not find '{key}' row in the status summary table")
    return plan + pat.sub(lambda m: f"{m.group(1)} {value} {m.group(2)}", status, count=1)
...
def set_wp_state(text: str, wp: str, state: str) -> str:
    ...
    if not pat.search(status):
        sys.exit(f"Work package {wp} not found in the Status section")
    return plan + pat.sub(lambda m: f"{m.group(1)} {state} {m.group(2)}", status, count=1)
...
def cmd_board(args: argparse.Namespace) -> None:
    paths = sorted(MODULES_DIR.glob("M[0-9][0-9]-*.md"))
    if not paths:
        sys.exit("No workstream files found.")
    header = ["Workstream", "Owner", "Phase", "State", "WPs done", "Active WPs", "Blockers", "Last updated"]
    rows = []
    for p in paths:
        s = parse_status(p)
        ...
        rows.append([
            f'{sm.get("Module", p.stem[:3])} {p.stem[4:]}', sm.get("Owner", "?"), sm.get("Phase", "?"),
...
def cmd_log(args: argparse.Namespace) -> None:
    ...
    if args.module and action == "PUSH":
        print("Remember: commit the workstream file change together with your code before pushing.")
...
def cmd_changes(args: argparse.Namespace) -> None:
    ...
    contract_files = git("diff", "--name-only", since, "HEAD", "--", "contracts", "docs/architecture/contracts").splitlines()
    if contract_files:
        print("\n!! Contract / catalog changes: check whether you consume these:")
        for f in contract_files:
            print(f"   - {f}")

    diff = git("diff", "-U0", since, "HEAD", "--", "docs/development/workstreams/*.md")
    current = None
    lines = []
    for ln in diff.splitlines():
        if ln.startswith("+++ b/"):
            current = Path(ln[6:]).stem
        elif ln.startswith("+- ") and current:
            lines.append(f"   [{current}] {ln[3:]}")
    if lines:
        print("\nNew workstream status log lines:")
        print("\n".join(lines))
...
def main() -> None:
    ...
    lg = sub.add_parser("log", help="record a push/pull (workstream status and/or your sync log)")
    ...
    lg.add_argument("--module", help="workstream id, e.g. M02")
    ...
    if args.cmd == "log" and args.action.upper() == "PUSH" and not args.module:
        print("Note: PUSH without --module only updates your sync log, not a workstream status.")
    args.func(args)
